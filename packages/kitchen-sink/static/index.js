...
function appendSection(title, contentBuilder, snippet = null) {
  if (currentCategoryNavigationContainer == null) {
    throw new Error(
      'No category navigation container.' +
  ...
  if (snippet) {
    ...
    snippetContainer.innerHTML = highlightKeywords(
      snippet.replace(/\n/g, '<br>'),
    );
...
// Marker function for extracting test snippets
function test(str) {
  return str;
}
...
withinCategory('Actions', () => {
  appendSection(
    ...
    test(`When I visit "{{basePath}}/forward.html"
  ...
  appendSection(
    ...
    test(`Given I visit "{{basePath}}"
  when I hover button4
  then message7 should be visible`),
  );
